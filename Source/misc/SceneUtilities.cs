...
namespace Melt
{
    ...
    {
        public static void CompareObjects(List<uint> sceneIdsEoR, List<uint> sceneIdsDM)
        {
            StreamWriter outputFile = new StreamWriter(new FileStream("SceneObjectsList.txt", FileMode.Create, FileAccess.Write));
            if (outputFile == null)
            {
                Console.WriteLine("Unable to open SceneObjectsList.txt");
                return;
            }

            SortedDictionary<uint, uint> translationMap = new SortedDictionary<uint, uint>();
            foreach (var sceneIdEntry in sceneIdsDM)
            {
                ...
                for(int i = 0; i < sceneEoR.Objects.Count; i++)
                {
                    var objEoR = sceneEoR.Objects[i];
                    var objDM = sceneDM.Objects[i];

                    if(objEoR.ObjId != objDM.ObjId)
                        Console.WriteLine($"SceneId: {sceneEoR.Id}: Object id mismatch.");

                    if(objEoR.ObjId != 0)
                        outputFile.WriteLine($"readdat -f \"portal.dat\" {objEoR.ObjId.ToString("x8")}");
                }
            }
